        ...
        if turn == 0:
            ...
            if is_valid_location(board, col):
                ...
                if winning_move(board, 1):
                    print("Player 1 wins!!!")
                    Player1_wins += 1
                    Player1_win_count.append(Player1_wins)
                    game_over = True
...
x = [x for x in range(len(Player1_win_count))]
pyplot.plot(x, Player1_win_count, label = "Total Number of Wins")
pyplot.title('Total Wins for Q-Learning Agent vs Random Agent, over sample of 500 games')
pyplot.legend()
pyplot.show()
